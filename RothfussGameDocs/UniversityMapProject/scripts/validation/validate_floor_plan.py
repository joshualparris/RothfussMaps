...
def validate_containment(data, issues):
    footprint = data.get("footprint", {})
    footprint_polygon = footprint.get("polygon")
    if not isinstance(footprint_polygon, list):
        return

    for group in ("rooms", "stairs"):
...
def validate_room_overlaps(data, issues):
    rooms = data.get("rooms", [])
        ...
        for right_index in range(left_index + 1, len(rooms)):
            ...
            if overlap:
                add(
                    issues,
                    "error",
                    f"rooms[{left_index}], rooms[{right_index}]",
                    f"Room polygons overlap with positive area: {left.get('id')} and {right.get('id')}.",
                )


def validate_door_references(data, issues):
    room_ids = {room.get("id") for room in data.get("rooms", []) if isinstance(room, dict)}
    allowed = room_ids | {"exterior"}
    footprint_polygon = data.get("footprint", {}).get("polygon", [])

    exterior_doors = 0
    for index, door in enumerate(data.get("doors", [])):
        path = f"doors[{index}]"
        validate_certified_item(door, path, issues)

        connects = door.get("connects")
        if not isinstance(connects, list) or len(connects) != 2:
            add(issues, "error", f"{path}.connects", "Door must connect exactly two spaces.")
        else:
            if "exterior" in connects:
                exterior_doors += 1
            for ref in connects:
                if ref not in allowed:
                    add(issues, "error", f"{path}.connects", f"Door references unknown space '{ref}'.")

        line = door.get("line")
        if not isinstance(line, dict):
            add(issues, "error", f"{path}.line", "Door must define a line with start and end points.")
            continue
        ...
        if start == end:
            add(issues, "error", f"{path}.line", "Door line has zero length.")

        if footprint_polygon:
            if not point_in_polygon(start, footprint_polygon, include_boundary=True):
                add(issues, "error", f"{path}.line.start", "Door start sits outside footprint.")
            if not point_in_polygon(end, footprint_polygon, include_boundary=True):
                add(issues, "error", f"{path}.line.end", "Door end sits outside footprint.")
            if connects and "exterior" in connects:
                if not any(on_segment(a, b, start) and on_segment(a, b, end) for a, b, _ in polygon_edges(footprint_polygon)):
                    add(issues, "warning", f"{path}.line", "Exterior door is not on a single footprint edge.")

    expected_public_entrances = data.get("footprint", {}).get("public_entrances")
    if isinstance(expected_public_entrances, int) and exterior_doors != expected_public_entrances:
        add(
            issues,
            "error",
            "footprint.public_entrances",
            f"Expected {expected_public_entrances} exterior door(s), found {exterior_doors}.",
        )


def validate_stair_references(data, issues):
    room_ids = {room.get("id") for room in data.get("rooms", []) if isinstance(room, dict)}
    floor_id = data.get("floor", {}).get("id")

    for index, stair in enumerate(data.get("stairs", [])):
        path = f"stairs[{index}]"
        validate_certified_item(stair, path, issues)

        access_from = stair.get("access_from")
        if access_from not in room_ids:
            add(issues, "error", f"{path}.access_from", f"Stair access_from references unknown room '{access_from}'.")

        if stair.get("from_floor") != floor_id:
            add(issues, "warning", f"{path}.from_floor", "Stair from_floor does not match this floor id.")

        connects = stair.get("connects")
        if connects is not None:
            if not isinstance(connects, list) or len(connects) < 2:
                add(issues, "error", f"{path}.connects", "Stair connects must include at least source and target.")
            elif connects[0] not in room_ids:
                add(issues, "error", f"{path}.connects[0]", "First stair connection should reference a known room.")


def validate_draft_flags(data, issues):
    floor = data.get("floor", {})
    if floor.get("draft") is not True:
        add(issues, "error", "floor.draft", "Draft/source status must be explicit and true for this approximate blockout.")
    if floor.get("approximate_dimensions") is not True:
        add(issues, "error", "floor.approximate_dimensions", "Approximate dimensions must be explicitly flagged.")

    for group in ("rooms", "doors", "stairs"):
        for index, item in enumerate(data.get(group, [])):
...
def validate(data):
    ...
    if "schema_version" in data and data["schema_version"] != "canon_floor_plan_v02":
        add(issues, "warning", "schema_version", "Expected 'canon_floor_plan_v02'.")

    validate_certified_item(data.get("building", {}), "building", issues)
    validate_certified_item(data.get("footprint", {}), "footprint", issues)
    if isinstance(data.get("floor"), dict):
        validate_certainty(data["floor"].get("certainty"), "floor.certainty", issues)
    if isinstance(data.get("story_role"), dict):
        validate_certainty(data["story_role"].get("certainty"), "story_role.certainty", issues)
    ...
    for index, room in enumerate(data.get("rooms", [])):
        validate_certified_item(room, f"rooms[{index}]", issues)
    ...
    validate_room_overlaps(data, issues)
    validate_door_references(data, issues)
    validate_stair_references(data, issues)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Validate canon-safe University floor plan JSON.")
    parser.add_argument("json_path", type=Path, help="Path to a canon_floor_plan_v02 JSON file.")
